 ...
 */
class YibaoKjController extends PayController
{

    //支付方式code
    private $code = '';

    private $desc = '易宝-银联快捷';
    ...
    public function notifyurl()
    {
        ...
        if ($body["body"]['status'] == "1") {
            ...
            Log::record("易宝快捷支付异步通知：" . "交易成功！订单号：" . $pay_orderid, Log::INFO);
            exit("SUCCESS");
        } else {
            Log::record("易宝快捷支付异步通知：" . "交易失败！订单号：" . $pay_orderid . "，参数：". json_encode($body), Log::ERR);
            exit("FAIL");
        }
    }
