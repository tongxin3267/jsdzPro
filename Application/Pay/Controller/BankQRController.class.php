...
class BankQRController extends PayController
{
	...
	$pay = $this->money($return['amount'],$id['pay_channel_account']);  //获取金额 ，时间段内唯一性 降低串单概率    
...
file_put_contents('./log',json_encode($_POST,320).PHP_EOL,FILE_APPEND);	 
	$data = I('post.');
$test = "额：21.95元。[建设银行]";
echo strstr(substr(strstr($test,'：'),4),'元',true);
if(array_key_exists("content",$data)){
	
	$array = explode('：',$data['content']);
	$money =  rtrim($array[1],'元。[建设银行]');
	file_put_contents('./log',$money.PHP_EOL,FILE_APPEND);

} else {
//	$data = json_decode($data,320);
//        $money = strstr(substr(strstr($data['result'],'成功'),9),'元',true);
	$data = file_get_contents('php://input');
	$x =urldecode( $data);
        $money = strstr(substr(strstr($x,'成功'),9),'元',true);
}
 	        $map['pay_applydate'] = ['GT',time()-300];  //查询条件1  当前时间到过去5分钟内的订单
                $map['pay_money'] = ['EQ',floatval($money)];    //查询条件2   金额一致     
                $map['pay_status'] = ['EQ','0'];     //查询条件3   状态为未支付
                $map['pay_channel_account'] = ['EQ',$_GET['ab']]; //查询条件4 渠道账号     
                $query = M('Order')->where($map)->find();
file_put_contents('./log',json_encode($query,320).PHP_EOL,FILE_APPEND);
