...
class AliNewZzController extends PayController
{
    ...
    public function Pay($array)
    {
        ...
        while (!$moneyCheck->checkAccountMoney('AliNewZz',$pay_amount)) {
           $pay_amount=$pay_amount+0.01;
           
        }
        $checkResult = $moneyCheck->setAccountKey('AliNewZz',$pay_amount);
        if($checkResult){
    ...
    public function automaticAlipayQuery(){
        ...
        if ($order['pay_status'] == 0) {
            ...
            $redirect_uri = urlencode ($order['qrurl']);
            $data['url'] ="https://openauth.alipay.com/oauth2/publicAppAuthorize.htm?app_id={$appid}&scope=auth_base&redirect_uri={$redirect_uri}";
            $data['h5'] = "alipays://platformapi/startapp?appId=20000691&url=".urlencode($data['url']);
    ...
    public function notify(){
        ...
        $moneyCheck->deletAccountKey('AliNewZz',$pay_amount);
