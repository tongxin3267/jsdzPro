...
<body style="background-color:#f9f9f9">
    <form action="<?php echo U("Account/dochongzhi");?>" method="post" autocomplete="off">
        <!--弹窗开始-->
        ...
        <div class="w1080 order-amount12" style="border-radius: 1em;">
            <ul class="order-amount12-left">
                <li>
                    <span>商品名称：</span>
                    <span>
                        <?php echo ($data["product_name"]); ?></span>
                </li>
                <li>
                    <span>订单编号：</span>
                    <span>
                        <?php echo ($data["pay_orderid"]); ?></span>
                </li>
            </ul>
            <div class="order-amount12-right">
                <span>充值金额：</span>
                <strong><input type="text" name="amount" value="<?php echo ($data["pay_amount"]); ?>"></strong>
                <span>元</span>
            </div>
        </div>
        <!--支付方式-->
        <input type="hidden" name="orderid" value="<?php echo ($data["pay_orderid"]); ?>">
        <input type="hidden" name="mchid" value="<?php echo ($data["mchid"]); ?>">
        <input type="hidden" name="bankcode" id="bankcode" value="">
        <div class="w1080 PayMethod12" style="border-radius: 1em;">
            <div class="row">
                <h2>支付方式</h2>
                <ul>
                    <?php if(is_array($products)): $key = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$product): $mod = ($key % 2 );++$key;?><li class="pay_li">
                            <input value="<?php echo ($product["id"]); ?>" name="bankcodes"  type="radio">
        ...
        <div class="w1080 immediate-pay12" style="border-radius: 1em; padding-top:1em; padding-bottom: 1em;padding-right: 1em;">
            <div class="immediate-pay12-right">
                <span>需支付：<strong>
                        <?php echo ($data["pay_amount"]); ?></strong>元</span>
