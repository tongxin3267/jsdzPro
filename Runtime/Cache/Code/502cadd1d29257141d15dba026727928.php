				...
				<?php if(is_array($products)): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i;?><tr>
					...
					<td>
						<div class="layui-input-inline" style="width: 100px;">
							<input type="text" name="u[<?php echo ($p["id"]); ?>][t0feilv]" placeholder=""
								   onchange="user_rate($(this).val(),'<?php echo ($p["id"]); ?>',0)"
								   autocomplete="off"
								   class="layui-input" value="<?php echo ($p["t0feilv"]); ?>">
						</div>
					</td>
					<td>
						<div class="layui-input-inline" style="width: 100px;">
					...
					<td>
						...
						<input type="text" name="u[<?php echo ($p["id"]); ?>][feilv]" placeholder=""
									 onchange="user_rate($(this).val(),'<?php echo ($p["id"]); ?>',1)"
									 autocomplete="off"
...
function user_rate(rate,pid,t) {
    ...
    $.ajax({
        ...
        type:"post",
        data:'feilv='+rate+'&pid='+pid+'&t='+t+'&userid='+userid,
        success:function(res){
            if(res.status){
                layer.msg(res.msg? res.msg :"费率设置不合法！", {icon: 5});
            }
        }
    });
}
